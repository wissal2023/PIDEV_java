...
ObservableList<String> allergiesList = FXCollections.observableArrayList("none","Pollen","Dust","Peanuts","Shellfish","Mold");
ObservableList<String> pathologiesList = FXCollections.observableArrayList("none","Asthma", "Diabetes", "Hypertension", "Cancer", "Arthritis");
    ...
    txt_cin.setOnKeyPressed(event -> {
        if (event.getCode() == KeyCode.ENTER) {
            ...
            try {
                ...
                String query = "SELECT u.nom, u.num_tel FROM user u WHERE u.roles LIKE '[\"ROLE_PATIENT\"]' AND u.cin =?";
    ...
    if (event.getCode() == KeyCode.ENTER) {
        ...
        try {
            ...
            String query = "SELECT email , nom, prenom, adresse, num_tel, date_de_naissance, sexe, num_securite_sociale " +
               "FROM user " +
               "WHERE roles LIKE '[\"ROLE_PATIENT\"]' AND cin=?";
            PreparedStatement preparedStatement = cnx.prepareStatement(query);
            preparedStatement.setString(1, cin);
            ResultSet resultSet = preparedStatement.executeQuery();
            if (resultSet.next()) {
